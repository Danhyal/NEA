 ...
 * @author Samuel Audet
 */
public abstract class MyOpenCVFrameConverter<F> extends FrameConverter<F> {
    ...
    public static class ToIplImage extends MyOpenCVFrameConverter<IplImage> {
    ...
    public static class ToMat extends MyOpenCVFrameConverter<Mat> {
        @Override public Frame convert(Mat mat) {
            return super.convert(mat); }
        @Override public Mat convert(Frame frame) {
            return convertToMat(frame); }
    }

    public static class ToOrgOpenCvCoreMat extends MyOpenCVFrameConverter<org.opencv.core.Mat> {
        @Override public Frame convert(org.opencv.core.Mat mat) {
            return super.convert(mat); }
        @Override public org.opencv.core.Mat convert(Frame frame) {
            return convertToOrgOpenCvCoreMat(frame); }
    }
    public static class ToGMat extends MyOpenCVFrameConverter<GpuMat> {
        @Override public Frame convert(GpuMat mat) {
            return super.Gconvert(mat); }
        @Override public GpuMat convert(Frame frame) {
            return convertToGMat(frame); }
    }
     public static class ToUMat extends MyOpenCVFrameConverter<UMat> {
        @Override public Frame convert(UMat mat) {
            return super.Uconvert(mat); }
        @Override public UMat convert(Frame frame) {
            return convertToUMat(frame); }
    }
    ...
    public UMat convertToUMat(Frame frame) {
        if (frame == null || frame.image == null) {
            return null;
        } else if (frame.opaque instanceof UMat) {
            return (UMat) frame.opaque;
        } else if (!isEqual(frame, mat)) {
            int depth = getMatDepth(frame.imageDepth);
            Mat temp = depth < 0 ? null : new Mat(frame.imageHeight, frame.imageWidth, CV_MAKETYPE(depth, frame.imageChannels),
                    new Pointer(frame.image[0].position(0)), frame.imageStride * Math.abs(frame.imageDepth) / 8);
            umat=temp.getUMat(1);
        }
        return umat;
    }
    public GpuMat convertToGMat(Frame frame) {
        if (frame == null || frame.image == null) {
            return null;
        } else if (frame.opaque instanceof GpuMat) {
            return (GpuMat) frame.opaque;
        } else if (!GisEqual(frame, gmat)) {
            int depth = getMatDepth(frame.imageDepth);
            gmat = depth < 0 ? null : new GpuMat(frame.imageHeight, frame.imageWidth, CV_MAKETYPE(depth, frame.imageChannels),
                    new Pointer(frame.image[0].position(0)), frame.imageStride * Math.abs(frame.imageDepth) / 8);
        }
        return gmat;
    }
    ...
    public Frame Gconvert(GpuMat mat) {
        if (mat == null) {
            return null;
        } else if (!GisEqual(frame, mat)) {
            frame = new Frame();
            ...
            frame.image = new Buffer[] { mat.asBuffer() };
        }
        frame.opaque = mat;
        return frame;
    }
    public Frame Uconvert(UMat mat) {
        if (mat == null) {
            return null;
        } else if (!UisEqual(frame, mat)) {
            frame = new Frame();
            frame.imageWidth = mat.cols();
            frame.imageHeight = mat.rows();
            frame.imageDepth = getFrameDepth(mat.depth());
    ...
    static boolean GisEqual(Frame frame, GpuMat mat) {
        return mat != null && frame != null && frame.image != null && frame.image.length > 0
                && frame.imageWidth == mat.cols() && frame.imageHeight == mat.rows()
                && frame.imageChannels == mat.channels() && getMatDepth(frame.imageDepth) == mat.depth()
                && new Pointer(frame.image[0].position(0)).address() == mat.address();
    }
    static boolean UisEqual(Frame frame, UMat mat) {
        return mat != null && frame != null && frame.image != null && frame.image.length > 0
                && frame.imageWidth == mat.cols() && frame.imageHeight == mat.rows()
                && frame.imageChannels == mat.channels() && getMatDepth(frame.imageDepth) == mat.depth()
                && new Pointer(frame.image[0].position(0)).address() == mat.address();
